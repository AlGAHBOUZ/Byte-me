...
void Game::InitWindow(){
    ...
    if (!font.loadFromFile("img/Behila.ttf"))
    {
        std::cout << "error\n";
    }
...
void Game::MoveFromVectorsCards() {
    ...
    for(int vec = 0; vec < 7; vec++){
        ...
        else  if(!here && card_vectors_array[vec].size() != 0 &&  !card_vectors_array[vec][idx]->getHidden() && idx != vec_size-1  && card_vectors_array[vec][idx]->getGlobalBounds().contains(
                window->mapPixelToCoords(sf::Mouse::getPosition(*window)))){
            std::cout << card_vectors_array[vec][idx]->getNumber() << "  " << card_vectors_array[vec][idx]->getSymbol() << std::endl;
            std::cout << idx << "  " << vec_size-1 << std::endl;
            MoveMultipleCards(card_vectors_array[vec][idx]);
            break;
        }
    }
}
...
void Game::MoveMultipleCards(Card *playing_card) {
    for (int vec = 0; vec < 7; vec++) {
        int sz = card_vectors_array[vec].size();
        int idx;
        bool card_moved;
        ...
        for (int i = 0; i < sz; i++) {
            ...
            if (playing_card == card_played && !card_played->getHidden()) {
                for (int vec2 = 0; vec2 < 7; vec2++) {
                    ...
                    if (sz2 != 0) {
                        ...
                        if (card_vectors_array[vec2][sz2 - 1]->getNumber() ==
                            ...
                            std::cout << "here\n";
                            std::cout << card_vectors_array[vec2][sz2 - 1]->getNumber() << "  " << card_vectors_array[vec2][sz2 - 1]->getSymbol() << std::endl;
                            card_moved = 1;
                            vec_indx = vec2;
                            break;
                        }
        ...
        if(card_moved){
            for (int i = idx; i < sz; i++) {
                card_vectors_array[vec_indx].push_back(card_vectors_array[vec][i]);
            }
            for(int i = sz-1; i > idx-1; i--){
                card_vectors_array[vec].pop_back();

            }
            int size = card_vectors_array[vec].size();
            if (size != 0)
                card_vectors_array[vec][size - 1]->setHidden(0);
            break;
        }
