...
void Game::HintBtn() {
    hint_btn = new Button("Hints!", {95, 95}, 12, sf::Color::Blue, sf::Color::Black);
    float btn_x_pos = 1000;
    float btn_y_pos = 0;
    hint_btn->setPosition({btn_x_pos, btn_y_pos});
    hint_shape.setSize({100, 100});
    hint_shape.setPosition({btn_x_pos, btn_y_pos});
    // Load Button image
    if (!hint_texture.loadFromFile("img/start.png")) {
        std::cout << "Load failed" << std::endl;
        system("pause");
    }
...
// some habd stuff to use in hint
...
//start of habd of hint
    sf::Text hintsth("", font);
    // M_K change the position
    hintsth.setPosition(70,855);
